...
def create_featurizer(
    ...
    Args:
        ...
        feat_kind: ``"wavlm"`` | ``"hubert"`` | ``"wav2vec2"``. If None, uses env ``SPOOFDET_FEAT``.
        hubert_ckpt: Path to fairseq HuBERT checkpoint (HuBERT only).
        wavlm_model: Hugging Face model id (WavLM only).
        wav2vec2_model: Hugging Face model id (wav2vec 2.0 only).
    """
    kind = (feat_kind or _env("SPOOFDET_FEAT", "SAFEAR_FEAT", "hubert")).strip().lower()
    if kind in ("hubert", "h", "fairseq_hubert"):
        from inference.hubert_featurizer import HubertFeaturizer

        ckpt = hubert_ckpt or _env(
            "SPOOFDET_HUBERT", "SAFEAR_HUBERT", str(_REPO_ROOT / "model_zoos" / "hubert_base_ls960.pt")
        )
    ...
    if kind in ("wavlm", "wavlm-base", "w", ""):
        ...
        name = wavlm_model or _env("SPOOFDET_WAVLM", "SAFEAR_WAVLM", "microsoft/wavlm-base")
    ...
    if kind in ("wav2vec2", "wav2vec", "w2v2", "facebook_wav2vec2"):
        ...
        name = wav2vec2_model or _env("SPOOFDET_WAV2VEC2", "SAFEAR_WAV2VEC2", "facebook/wav2vec2-base")
        return Wav2Vec2Featurizer(model_name=name, device=device)
    raise ValueError(f"Unknown feat_kind / SPOOFDET_FEAT={kind!r}; use 'wavlm', 'hubert', or 'wav2vec2'")
