#!/usr/bin/env bash
# 续跑：heads8 → layers1 → layers3 → bilstm（no_pe 已有 test_results.json 则跳过）
# 用法:
...
echo "[START] Remaining supplement ablations at $(date '+%F %T')"
...
"$PYTHON" scripts/aggregate_test_results.py --exps-dir Exps --out-dir Exps/_summary 2>&1 | tee -a "${LOG_DIR}/aggregate.log"

echo "[DONE] Remaining supplement runs finished at $(date '+%F %T')"
